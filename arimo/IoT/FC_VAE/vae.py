...
def generate_ewma_resample1D_ad_score(
    df, id_col="equipment_instance_id", date_time_col="date_time"
):
...
def ewma_10d(tmp_df, avg_days=8):
    for col in tmp_df.columns.values:
        ewma = pd.Series.ewm
        fwd = ewma(
            tmp_df.loc[:, col], span=avg_days
        ).mean()  # take EWMA in fwd direction
        bwd = ewma(
            tmp_df.loc[:, col][::-1], span=avg_days
        ).mean()  # take EWMA in bwd direction
        filtered = np.vstack(
            (fwd.values, bwd[::-1].values)
        )  # lump fwd and bwd together
        filtered = np.mean(filtered, axis=0)  # average
        tmp_df[
            "ewma8__dailyMean__"
            + col
            + "__lpg_water_system__old_filter_power_gen_on__VAE"
        ] = filtered
    return tmp_df
...
class PredictiveMaintenanceVAE(object):
    def __init__(
        ...
        if self.pos_enc:
            self.position_encoding = self.get_position_encoding(
                num_positions=n_window, num_features=input_dim, min_val=10000
            )
    ...
    def train(
        ...
        with self.tf_graph.as_default():
            x_train = tf.keras.Input(
                name="x_placeholder",
                shape=[n_feature * n_window],
                dtype=tf.dtypes.float32,
            )
            x_val = tf.keras.Input(
                name="x_placeholder_val",
                shape=[n_feature * n_window],
                dtype=tf.dtypes.float32,
            )
            (
            ...
            running_vars = tf.compat.v1.get_collection(
                tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope="vae/my_mae"
            )
            print("running_vars:", running_vars)
            running_vars_initializer = tf.compat.v1.variables_initializer(
            ...
            with tf.compat.v1.Session() as sess:
                ...
                for epoch in range(num_epochs):
                    for i in range(batchs_train):
                        ...
                        _, _, global_step_value = sess.run(
                            [
                                train_op,
                                train_mae_update_op,
                                # val_mae_update_op,
                        ...
                        if i % summary_step == 0:
                            ...
                            if n_window > 1:
                                x_val_datetime = sess.run(
                                    x_val_tensor[tfr_datetime_key]
                                )
                                x_val_feature = x_val_feature.reshape(
                                    [-1, n_feature * n_window]
                                )
                            ...
                            _ = sess.run(
                                val_mae_update_op, feed_dict={x_val: x_val_feature}
                            )
                            (
                                loss_value,
                                mae_value,
                        ...
                        if i % printing_step == 0:
                            ...
                            format_str = (
                                "%s epoch: %d batch: %d, mae: %.5f, mae_val: %.5f, loss: %.5f, loss_val: %.5f (%.1f examples/sec; %.3f "
                                "sec/batch)"
                            )
                            print(
                                format_str
    ...
    def generate_summary_bak(
        self, df, id_col, date_time_col, prob_col, sensor_cols, threshold, top_n=10
    ):
        gb = df.groupby(id_col)
        list_final = [gb.get_group(x) for x in gb.groups]
        df_list = []
        for idx, df_one in enumerate(list_final):
            ...
            if temp3.shape[0] > 0:
                ...
                if sensor_cols:
                    df_list.append(
                        temp4[[id_col, date_time_col, prob_col] + list(sensor_cols)]
                    )
                else:
    ...
    def get_threshold(self, df, id_col, date_time_col, resample="1h", portion=0.001):
        ...
        for df_one in list_final:
            avg_prob = (
                df_one.set_index(date_time_col).resample(resample).mean().prob.values
            )
    ...
    def get_range_group(self, df, datetime_col):
        ...
        for idx in idx_list[1:]:
            if (
                df.loc[idx, datetime_col] - df.loc[end_idx, datetime_col]
    ...
    def get_range_summary_for_one_event(
        self, df, id_col, datetime_col, sensor_cols, prob_col, top_n=10
    ):
        score_dict = {}
        ...
        for key, value in reversed(
            sorted(score_dict.items(), key=lambda kv: (kv[1], kv[0]))
        ):
            sensors.append(key)
        #     print "%s: %s" % (key, value)
        summary = [
    ...
    def get_range_summary_for_one_instance(
        self, df, group_list, id_col, datetime_col, sensor_cols, prob_col, top_n=10
    ):
        summary_list = []
    ...
    def get_range_summary_for_one_instance_only_prob(
        self, df, group_list, id_col, datetime_col, prob_col
    ):
        summary_list = []
        for idxs in group_list:
            df_group = df.loc[idxs[0] : idxs[1]]
            summary = [
                df_group[id_col].iloc[0],
            ...
            summary_list.append(summary)
        df_range_summary = pd.DataFrame(
            summary_list, columns=[id_col, "from_date", "to_date", "score"]
        )
        return df_range_summary

    def get_range_summary_mlt(
        self, df, id_col, datetime_col, sensor_cols, prob_col, top_n=10
    ):
        gb = df.groupby(id_col)
        list_final = [gb.get_group(x) for x in gb.groups]
        ...
        pool = Pool().imap
        individual_range_summary_results = pool(
            get_individual_range_summary, list_final
        )
    ...
    def score(
            ...
            with tf.compat.v1.Session(
                ...
                for i in range(batchs_per_epoch):
                    ...
                    if individual_sensor_prob:
                        x_log_prob_o, prob_for_individual_sensor_o, z_mean_o = sess.run(
                            [x_log_prob, prob_for_individual_sensor, z_mean],
                            feed_dict={x: x_score_feature},
                        )
                        sensor_avg_list.extend(prob_for_individual_sensor_o)
                    else:
                        x_log_prob_o, z_mean_o = sess.run(
                            [x_log_prob, z_mean], feed_dict={x: x_score_feature}
                        )
                    if n_window > 1:
        ...
        if len(z_mean_avg_array[0]) > 2:
            ...
            z_pca = np.squeeze(
                np.transpose(np.array([z_mean_avg_array, z_mean_avg_array]))
            )
        else:
        ...
        if individual_sensor_prob:
            ...
            result_df = pd.DataFrame(
                data=result_np, columns=["prob"] + ["z1", "z2"] + sensor_cols
            )
        else:
            ...
            sensor_cols = None
        result_np = None
        df_id_n_datetime = pd.DataFrame(
            {"equipment_instance_id": id_list, "date_time": datetime_list}
        )
        id_list = None
        datetime_list = None
        ...
        df_whole["date_time"] = pd.to_datetime(
            df_whole["date_time"].str.decode("UTF-8")
        )
        ...
        print(os.path.join(save_path, "ad_score.csv"))
    ...
    def get_position_encoding(self, num_positions, num_features, min_val=10000):
        ...
        angle_rads = get_angles(
            np.arange(0, num_positions)[:, np.newaxis],
    ...
    def get_train_val_iterator_from_tfr_in_s3_new(
        self,
        folder_path,
        batch_size,
        tfr_id_key,
        ...
        if pre_fix[-1] != "*":
            pre_fix += "*"
        filenames = [
            "s3://" + k for k in s3_fs.glob(os.path.join(folder_path, pre_fix))
        ]
        ...
        _tmp_full_dataset = lambda x: self._decode(
            x, decode_key=tfr_feature_key, n_feature=n_feature
        )
    ...
    def get_train_val_iterator_from_tfr_in_local(
        self,
        folder_path,
        batch_size,
        tfr_id_key,
    ...
    def get_train_val_iterator_from_tfr_in_s3(
        ...
        response = client.list_objects(
            Bucket=bucket_name, Prefix=os.path.join(tfr_path, pre_fix)
        )
        # print "response:", response
        filenames = [
            os.path.join("s3://", bucket_name, item["Key"])
    ...
    def get_train_val_iterator_from_tfr(
        self,
        folder_path,
        batch_size,
        tfr_id_key,
    ...
    def _decode2(
        self, serialized_example, id_key, datetime_key, feature_key, n_input=None
    ):
        unserialized = tf.io.parse_single_example(
            serialized_example,
            features={
                id_key: tf.io.FixedLenFeature([], dtype=tf.string),
    ...
    def get_iterator_from_tfr_in_s3(
        ...
        _tmp_dataset = lambda x: self._decode2(
            x, tfr_id_key, tfr_datetime_key, tfr_feature_key, n_input=n_input
        )
    ...
    def get_iterator_from_tfr_in_local(
        self,
        folder_path,
        batch_size,
        tfr_id_key,
        ...
        if pre_fix[-1] != "*":
            pre_fix += "*"
        filenames = sorted(glob.glob(os.path.join(folder_path, pre_fix)))[
            tfr_score_index[0] : tfr_score_index[1]
        ]
    ...
    def save_df_to_csv_in_s3(self, dataframe, save_path, csv_name):
        ...
        s3_resource.Object(bucket_name, os.path.join(file_path, csv_name)).put(
            Body=csv_buffer.getvalue()
        )

    def get_valid_window_batch(
        self, x_feature, x_datetime, n_window, batch_size, interval
    ):
        valid_window_idx = []
        for i in range(batch_size):
            datetime_start = datetime.datetime.strptime(
                x_datetime[i][0], "%Y-%m-%d %H:%M:%S"
            ).replace(second=0)
            datetime_end = datetime.datetime.strptime(
                x_datetime[i][-1], "%Y-%m-%d %H:%M:%S"
            ).replace(second=0)
