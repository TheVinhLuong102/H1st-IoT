...
class BaseModelGraph(object):
  ...
  def setup_input_placeholders_and_embedding_lookup(self,
    ...
    if input_tensor_transform_fn:
      model_input = tf.Print(model_input, [model_input], "input_data pre input_tensor_transform_fn  = ", summarize=input_size, first_n=2)
...
class ClassifierModelGraph(BaseModelGraph):
  ...
  def __init__(self,
    ...
    if config.embedding_size:
      outputs = self.network_topology(model_input, input_size + config.embedding_size, target_size, self.apply_dropout)
    else:
